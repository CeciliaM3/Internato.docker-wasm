...
$SELECTED_EXECUTABLE \
  && docker ps -a -q --filter ancestor=statistics_calc_executable \
    | xargs -r docker rm -f \
  && podman ps -a -q --filter ancestor=statistics_calc_executable \
    | xargs -r podman rm -f

$SELECTED_WASM_NODE \
  && docker ps -a -q --filter ancestor=statistics_calc_wasm_node \
    | xargs -r docker rm -f \
  && podman ps -a -q --filter ancestor=statistics_calc_wasm_node \
    | xargs -r podman rm -f

$SELECTED_WASM_BUN \
  && docker ps -a -q --filter ancestor=statistics_calc_wasm_bun \
    | xargs -r docker rm -f \
  && podman ps -a -q --filter ancestor=statistics_calc_wasm_bun \
    | xargs -r podman rm -f
...
$SELECTED_WASM_NODE \
  && (docker rmi statistics_calc_wasm_node 2>/dev/null || true) \
  && (podman rmi statistics_calc_wasm_node 2>/dev/null || true)

$SELECTED_WASM_BUN \
  && (docker rmi statistics_calc_wasm_bun 2>/dev/null || true) \
